...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, RouterLink],  // Mengimpor CommonModule dan ReactiveFormsModule agar bisa menggunakan fitur form dan direktif Angular standar
...
export class AuthComponent {
  ...
  apiUrl = ' https://be-iamfashion.vercel.app/api/auth/login';  // API login endpoint
  ...
  onAuth() {
    ...
    console.log('Sending login data (final check):', { email, password });
    ...
    this.http.post(this.apiUrl, { email, password }).subscribe({
      next: (response: any) => {
        ...
        if (response.token) {
          ...
          localStorage.setItem('userRole', response.role);  // Pastikan ini dilakukan setelah login// BISA ADMIN OR USER
          localStorage.setItem('userId', response.userId);  // Pastikan ini dilakukan setelah login// BISA ADMIN OR USER
